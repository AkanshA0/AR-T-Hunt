...

{
    ...
    public void saveRankCount(string child)
    {
      ...
      /*  FirebaseDatabase.RunTransactionAsync(transaction =>
        {
            return transaction.GetSnapshotAsync(reference.Child("rank")).ContinueWith((snapshotTask) =>
            {
                DocumentSnapshot snapshot = snapshotTask.Result;
                int newCount = snapshot.GetValue<int>("count") + 1;
                if (newCount != null)
                {
                    Dictionary<string, object> updates = new Dictionary<string, object>
            {
                { "count", newCount}
                    ...
                    transaction.Update(reference.Child("rank"), updates);
                    return true;
                }
                else
                {
